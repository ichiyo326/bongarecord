...
final class ThemeManager: ObservableObject {
    ...
    /// この上限で縮小し、端末容量の圧迫を防ぐ。
    nonisolated static let maxBackgroundDimension: CGFloat = 3000

    /// クロップ前の元画像を保存するときの上限解像度（長辺のピクセル数）。
    /// 「範囲を調整」で編集画面を開き直したときにズームインし直せる余地を残すため、
    /// 表示用の`maxBackgroundDimension`より大きめに取ってある。
    nonisolated static let maxSourceDimension: CGFloat = 4500
    ...
    private func refreshBackgroundDerivedState() {
        ...
        Task { [weak self] in
            let blurred = await Task.detached(priority: .userInitiated) {
                backgroundImage.blurred(radius: radius) ?? backgroundImage
            }.value
