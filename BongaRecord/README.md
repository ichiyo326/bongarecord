...
2. 設定:
    - Product Name: `BongaRecord`
    - Interface: **SwiftUI**
    - Language: **Swift**
    - Storage: **None**（SwiftDataを後から手動設定）
    - Include Tests: 任意
3. Bundle Identifier を一意な値に（例: `com.yourname.BongaRecord`）
...
4. iCloud のセクションで:
    - ✅ **CloudKit** にチェック
    - **Containers** で `+` を押し、自動生成された `iCloud.com.yourname.BongaRecord` をそのまま使う（または独自名で作成）
...
このフォルダのSwiftファイルすべてをXcodeの `BongaRecord` グループにドラッグ＆ドロップ。

```
BongaRecord/
├── BongaRecordApp.swift       ← Xcode自動生成と差し替え
├── Models/
│   ├── BattleRecord.swift
│   ├── PlayerInfo.swift
│   └── MasterData.swift
