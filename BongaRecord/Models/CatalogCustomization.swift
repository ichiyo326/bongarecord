...
final class CharacterPreference {
    ...
    /// ゲーム内の公式キャラランク（自己申告・自由入力）。
    /// 勝率から自動計算する練度ランク（S/A/B/C）とは別物で、
    /// プロフィール画面から直接タップして入力・編集できる。
    var girlRank: String = ""
...
enum Catalog {
    ...
    /// ガールランクの正規化：1〜150の整数文字列に丸める。未入力・不正値は既定の1。
    static func normalizedGirlRank(_ raw: String?) -> String {
        let n = Int((raw ?? "").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 1
        return String(min(max(n, 1), 150))
    }
    ...
    static func resolvedCharacters(prefs: [CharacterPreference],
        ...
        for c in MasterData.characters {
            ...
            result.append(DisplayCharacter(
                ...
                isHidden: p?.isHidden ?? false,
                isCustom: false,
                girlRank: normalizedGirlRank(p?.girlRank)
            ))
        }
