...
struct MatsubiNotificationView: View {
    ...
    // 開始時間：画面を開いた「今の時間」を初期値にする。
    @State private var startTime: Date = Date()

    // 終了時間：開始の1時間後を初期値にする。ただし1時間後が23時以降
    // （＝23:00〜、または日をまたぐ場合）になるときは23:59に丸める。
    @State private var endTime: Date = {
        let cal = Calendar.current
        let now = Date()
        let candidate = cal.date(byAdding: .hour, value: 1, to: now) ?? now
        let candidateHour = cal.component(.hour, from: candidate)
        let rolledToNextDay = !cal.isDate(candidate, inSameDayAs: now)
        if rolledToNextDay || candidateHour >= 23 {
            var comps = cal.dateComponents([.year, .month, .day], from: now)
            comps.hour = 23; comps.minute = 59; comps.second = 0
