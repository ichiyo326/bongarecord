...
struct BackgroundImageEditorView: View {
    ...
    private func confirm() {
        ...
        onConfirm(currentCrop() ?? originalImage)
    }
    ...
    private func currentCrop() -> UIImage? {
        guard let cgImage = originalImage.cgImage else { return nil }
        // コンテナサイズはbody内のGeometryReaderからしか取れないため、UIScreenの
        // 実サイズ（ポイント単位）で代用する。この画面は常に全画面表示（.ignoresSafeArea）
        // であるため、実際のコンテナサイズと一致する。
